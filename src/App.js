...
function App() {
    ...
    const fetchAllPublics = async () => {
      try {
        ...
        const response = await axios.get(
          `https://storisbro.com/api/vk/groups/?access_token=${accessToken}`,
          {
            headers: {
        ...
        logToBackend(
          `URL: https://storisbro.com/vk/groups/?access_token=${accessToken}`,
        );
        logToBackend(`Response DATA: ${JSON.stringify(response.data)}`);
      } catch (error) {
  ...
  useEffect(() => {
    ...
    const fetchSelectedPublics = async () => {
      try {
        const response = await axios.get(
          `https://storisbro.com/api/selected_groups/user_id=${userId}`,
          {
            headers: {
              Accept: "application/json",
            },
        ...
        logToBackend(`URL: https://storisbro.com/api/selected_groups/`);
        logToBackend(`Response DATA: ${JSON.stringify(response.data)}`);
      } catch (error) {
        console.error("Ошибка при загрузке сообществ", error);
      }
